...
app.use(express.json({ limit: '50mb' })); app.use("/uploads", express.static(UPLOAD_DIR));
...
const io = new Server(server, { 
    cors: { origin: "*", methods: ["GET", "POST", "OPTIONS"], allowedHeaders: "*" },
    allowEIO3: true,
...
// --- Admin API ---
app.post("/api/admin/login", (req, res) => { if (req.body.password === "towa.soko0406") { res.json({ token: jwt.sign({ id: "admin", nickname: "Admin", admin: true }, JWT_SECRET) }); } else { res.status(401).json({ error: "Invalid password" }); } });
app.post("/api/admin/database/cleanup", adminAuth, (req, res) => { db.serialize(() => { db.run(`DELETE FROM messages`); db.run(`DELETE FROM posts WHERE timestamp <= datetime('now', '-7 days')`); io.to("admin_monitor").emit("admin_update"); res.json({ success: true }); }); });
app.post("/api/admin/users/list", adminAuth, (req, res) => { db.all(`SELECT * FROM users ORDER BY status ASC, last_id_change DESC`, [], (err, rows) => res.json(rows || [])); });

app.post("/api/admin/users/approve", adminAuth, (req, res) => { 
...
app.post("/api/admin/users/ban", adminAuth, (req, res) => { db.run(`UPDATE users SET is_banned=1 WHERE id=?`, [req.body.id], () => { io.to("admin_monitor").emit("admin_update"); res.json({ success: true }); }); });
app.post("/api/admin/users/unban", adminAuth, (req, res) => { db.run(`UPDATE users SET is_banned=0 WHERE id=?`, [req.body.id], () => { io.to("admin_monitor").emit("admin_update"); res.json({ success: true }); }); });
app.post("/api/admin/users/delete", adminAuth, (req, res) => { const uid = req.body.id; db.serialize(() => { db.run(`DELETE FROM users WHERE id=?`, [uid]); db.run(`DELETE FROM friends WHERE user_id1=? OR user_id2=?`, [uid, uid]); db.run(`DELETE FROM posts WHERE user_id=?`, [uid]); }); io.to("admin_monitor").emit("admin_update"); res.json({ success: true }); });
app.post("/api/admin/notifications/send", adminAuth, (req, res) => { const { content } = req.body; db.all(`SELECT id FROM users WHERE status='approved'`, [], (err, users) => { if (!users || users.length === 0) return res.json({ success: true }); const stmt = db.prepare(`INSERT INTO notifications (id, user_id, type, from_user, content, is_read, timestamp) VALUES (?, ?, 'system', 'admin', ?, 0, CURRENT_TIMESTAMP)`); users.forEach(u => { stmt.run(uuidv4(), u.id, `【運営より】\n${content}`); }); stmt.finalize(() => { io.emit("new_notification"); res.json({ success: true }); }); }); });

app.post("/api/admin/posts/list", adminAuth, (req, res) => { db.all(`SELECT p.*, u.username, u.nickname, (SELECT COUNT(*) FROM post_likes WHERE post_id=p.id) as like_count FROM posts p JOIN users u ON p.user_id=u.id ORDER BY p.timestamp DESC LIMIT 100`, [], (err, rows) => res.json(rows || [])); });
app.post("/api/admin/posts/delete", adminAuth, (req, res) => { db.run(`DELETE FROM posts WHERE id=?`, [req.body.id], () => { io.emit("new_post_timeline"); io.to("admin_monitor").emit("admin_update"); res.json({ success: true }); }); });
app.post("/api/admin/password_requests/list", adminAuth, (req, res) => { db.all(`SELECT p.*, u.username, u.full_name FROM password_requests p JOIN users u ON p.user_id = u.id WHERE p.status='pending'`, [], (err, rows) => res.json(rows || [])); });
app.post("/api/admin/password_requests/approve", adminAuth, (req, res) => { db.get(`SELECT * FROM password_requests WHERE id=?`, [req.body.id], (err, r) => { if (r) db.run(`UPDATE users SET password=? WHERE id=?`, [r.new_password_hash, r.user_id], () => { db.run(`UPDATE password_requests SET status='approved' WHERE id=?`, [r.id], () => { io.to("admin_monitor").emit("admin_update"); res.json({ success: true }); }); }); }); });
app.post("/api/admin/password_requests/reject", adminAuth, (req, res) => { db.run(`UPDATE password_requests SET status='rejected' WHERE id=?`, [req.body.id], () => { io.to("admin_monitor").emit("admin_update"); res.json({ success: true }); }); });
app.post("/api/admin/messages/list", adminAuth, (req, res) => { db.all(`SELECT * FROM messages ORDER BY timestamp DESC LIMIT 200`, [], (err, rows) => res.json(rows || [])); });
app.post("/api/admin/messages/delete", adminAuth, (req, res) => { db.run(`DELETE FROM messages WHERE msg_id=?`, [req.body.id], () => { io.to("admin_monitor").emit("admin_update"); res.json({ success: true }); }); });
app.post("/api/admin/backups/list", adminAuth, (req, res) => { db.all(`SELECT b.id, b.user_id, u.username, u.nickname, b.timestamp FROM backups b JOIN users u ON b.user_id=u.id ORDER BY b.timestamp DESC`, [], (err, rows) => res.json(rows || [])); });
app.post("/api/admin/backups/view", adminAuth, (req, res) => { db.get(`SELECT data FROM backups WHERE id=?`, [req.body.id], (err, row) => res.json(row ? JSON.parse(row.data) : [])); });
app.post("/api/admin/notes/list", adminAuth, (req, res) => { db.all(`SELECT n.*, u.username, u.nickname, (SELECT COUNT(*) FROM note_likes WHERE note_user_id=n.user_id) as like_count FROM notes n JOIN users u ON n.user_id=u.id ORDER BY n.timestamp DESC LIMIT 100`, [], (err, rows) => res.json(rows || [])); });
app.post("/api/admin/likes/details", adminAuth, (req, res) => { 
...
app.post("/api/user/update_status_message", auth, (req, res) => { db.run(`UPDATE users SET status_message=? WHERE id=?`, [req.body.status_message, req.user.id], () => { io.emit("friend_update"); res.json({ success: true }); }); });
app.post("/api/user/update_icon", auth, upload.single("photo"), (req, res) => { const url = `/uploads/${req.file.filename}`; db.run(`UPDATE users SET icon_url=? WHERE id=?`, [url, req.user.id], () => { io.emit("friend_update"); res.json({ url }); }); });
app.post("/api/user/update_nickname", auth, (req, res) => { db.run(`UPDATE users SET nickname=? WHERE id=?`, [req.body.nickname, req.user.id], () => { io.emit("friend_update"); io.emit("new_post_timeline"); res.json({ success: true }); }); });
app.post("/api/user/update_username", auth, (req, res) => {
...
app.post("/api/user/request_password_change", auth, (req, res) => { db.run(`INSERT INTO password_requests VALUES (?,?,?,'pending',CURRENT_TIMESTAMP)`, [uuidv4(), req.user.id, bcrypt.hashSync(req.body.new_password, 8)], () => { io.to("admin_monitor").emit("admin_update"); res.json({ success: true }); }); });

app.post("/api/user/profile_status", auth, (req, res) => {
...
// --- Backup & Migrate ---
app.post("/api/backup/push", auth, (req, res) => {
...
app.post("/api/backup/pull", (req, res) => {
...
app.post("/api/notes/post", auth, (req, res) => { db.run(`INSERT OR REPLACE INTO notes (user_id, content, visibility, timestamp) VALUES (?, ?, ?, CURRENT_TIMESTAMP)`, [req.user.id, req.body.content, req.body.visibility || 'public'], () => { io.emit('notes_update'); res.json({ success: true }); }); });

app.post("/api/notes/list", auth, (req, res) => { 
...
app.post("/api/notes/like", auth, (req, res) => {
...
app.post("/api/notes/interactions", auth, (req, res) => {
...
app.post("/api/groups/list", auth, (req, res) => {
...
app.post("/api/groups/add_members", auth, (req, res) => {
...
// --- Social & Friends ---
app.post("/api/sync/messages", auth, (req, res) => { db.all(`SELECT * FROM messages WHERE room_id LIKE ? AND timestamp > ? ORDER BY timestamp ASC`, [`%${req.user.id}%`, req.body.since], (err, rows) => res.json({ messages: rows || [] })); });

app.post("/api/messages/delete_msg", auth, (req, res) => {
...
app.post("/api/messages/edit_msg", auth, (req, res) => {
    db.get(`SELECT * FROM messages WHERE msg_id=? AND user_id=?`, [req.body.msg_id, req.user.id], (err, m) => {
...
app.post("/api/friends/block_list", auth, (req, res) => {
...
app.post("/api/friends/unblock", auth, (req, res) => {
...
app.post("/api/search", auth, (req, res) => { const q = `%${req.body.query}%`; db.all(`SELECT id,username,nickname,icon_url,status_message FROM users WHERE status='approved' AND (username LIKE ? OR nickname LIKE ?) LIMIT 50`, [q, q], (err, users) => res.json({ users: users || [] })); });
app.post("/api/search/messages", auth, (req, res) => { const q = `%${req.body.query}%`; db.all(`SELECT * FROM messages WHERE room_id LIKE ? AND content LIKE ? AND type='text' AND is_deleted=0 ORDER BY timestamp DESC LIMIT 50`, [`%${req.user.id}%`, q], (err, msgs) => res.json({ messages: msgs || [] })); });
app.post("/api/notifications/list", auth, (req, res) => { db.all(`SELECT n.*, r.id as request_id, r.status as req_status FROM notifications n LEFT JOIN friend_requests r ON n.from_user=r.from_user AND r.to_user=n.user_id AND r.status='pending' AND n.type='friend_request' WHERE n.user_id=? ORDER BY n.timestamp DESC`, [req.user.id], (err, rows) => res.json(rows || [])); });
app.post("/api/notifications/read", auth, (req, res) => { db.run(`UPDATE notifications SET is_read=1 WHERE user_id=?`, [req.user.id], () => { io.to(req.user.id).emit("new_notification"); res.json({ success: true }); }); });

app.post("/api/messages/unread_count", auth, (req, res) => { 
...
app.post("/api/posts/delete", auth, (req, res) => { db.run(`DELETE FROM posts WHERE id=? AND user_id=?`, [req.body.post_id, req.user.id], () => { io.emit("new_post_timeline"); io.to("admin_monitor").emit("admin_update"); res.json({ success: true }); }); });

app.post("/api/posts/list", auth, (req, res) => {
    const q = `
...
app.post("/api/posts/like", auth, (req, res) => {
...
app.post("/api/posts/comments/list", auth, (req, res) => { db.all(`SELECT c.*, u.nickname, u.icon_url FROM post_comments c JOIN users u ON c.user_id = u.id WHERE c.post_id=? ORDER BY c.timestamp ASC`, [req.body.post_id], (err, rows) => res.json(rows || [])); });
app.post("/api/posts/comment", auth, (req, res) => { db.run(`INSERT INTO post_comments (id, post_id, user_id, content, image_url) VALUES (?,?,?,?,?)`, [uuidv4(), req.body.post_id, req.user.id, req.body.content, req.body.image_url||null], () => { io.emit("new_post_timeline"); res.json({ success: true }); }); });
app.post("/api/posts/interactions", auth, (req, res) => {
...
// --- スマホ連携 P2Pシグナリング ---
const smartphoneSessions = new Map();

app.post("/api/p2p/smartphone-offer", (req, res) => {
    const { session_id, offer } = req.body;
    if (!session_id || !offer) return res.status(400).json({ error: "Invalid request" });
    
    // セッション情報を保存
    smartphoneSessions.set(session_id, { offer, createdAt: Date.now() });
    
    // 30秒以上古いセッションを削除
    const now = Date.now();
    for (const [sid, data] of smartphoneSessions.entries()) {
        if (now - data.createdAt > 30000) {
            smartphoneSessions.delete(sid);
        }
    }
...
app.post("/api/p2p/smartphone-answer", auth, (req, res) => {
    const { session_id, answer } = req.body;
    if (!session_id || !answer) return res.status(400).json({ error: "Invalid request" });
    
    const session = smartphoneSessions.get(session_id);
    if (!session) return res.status(404).json({ error: "Session not found" });
    
    // ブロードキャストして接続されたクライアントに通知
    res.json({ offer: session.offer });
});

    s.on("smartphone-webrtc-offer", (data) => {
        const { session_id, offer } = data;
        if (!session_id || !offer) return;
        
        console.log('スマホOffer受信:', session_id);
        
        // スマホのセッション情報を保存
        if (!smartphoneSessions.has(session_id)) {
            smartphoneSessions.set(session_id, {});
        }
        const session = smartphoneSessions.get(session_id);
        session.offer = offer;
        session.smartphone_socket_id = s.id;
        
        // 接続されている全PCクライアントにOfferをブロードキャスト
        io.emit('smartphone-webrtc-offer', { session_id, offer });
    });

    s.on("smartphone-webrtc-answer", (data) => {
        const { session_id, answer } = data;
        if (!session_id || !answer) return;
        
        console.log('PC Answer受信:', session_id);
        
        // スマホセッションに Answer を保存
        if (smartphoneSessions.has(session_id)) {
            const session = smartphoneSessions.get(session_id);
            session.answer = answer;
            
            // スマホに Answer を返す（socket.idで特定のスマホに送信）
            if (session.smartphone_socket_id) {
                io.to(session.smartphone_socket_id).emit('smartphone-webrtc-answer-' + session_id, { answer });
            }
        }
    });

    s.on("smartphone-webrtc-ice", (data) => {
        const { session_id, candidate } = data;
        if (!session_id) return;
        
        // セッション内でどちらからのICE候補かを判定して転送
        if (smartphoneSessions.has(session_id)) {
            const session = smartphoneSessions.get(session_id);
            
            // スマホからの候補なら PCに、PCからの候補ならスマホに転送
            if (session.smartphone_socket_id === s.id) {
                // スマホからの候補 → PCに転送
                io.emit('smartphone-webrtc-ice', { session_id, candidate, from: 'smartphone' });
            } else {
                // PCからの候補 → スマホに転送
                if (session.smartphone_socket_id) {
                    io.to(session.smartphone_socket_id).emit('smartphone-webrtc-ice', { session_id, candidate, from: 'pc' });
                }
            }
        }
    });

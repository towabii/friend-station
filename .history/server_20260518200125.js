...
router.post('/edu/admin/users/:action', adminAuth, async (req, res) => {
  ...
  if (action === 'approve') {
    ...
    user.token = token;
    db.sessions.push({ token, user_id: user.id, created_at: now() });
    if (user.pendingToken && pendingSockets.has(user.pendingToken)) {
      const socket = pendingSockets.get(user.pendingToken);
      socket.emit('registration_approved', { token, user: sanitizeUser(user) });
      pendingSockets.delete(user.pendingToken);
    }
    await saveDb();
    emitToAdmin('admin_update');
    io.emit('friend_update');
...
io.on('connection', socket => {
  ...
  socket.on('answer_call', data => {
    ...
    if (!sockets) return;
    sockets.forEach(s => {
      s.emit(accept ? 'call_answered' : 'call_rejected', { session_id, caller_id: socket.userId, accept });
    });
  });
  ...
  socket.on('answer_text', data => {
    ...
    sockets.forEach(s => {
      if (accept) {
        s.emit('text_call_started', { session_id: `tc_${Date.now()}`, target_name: caller_name || authUser.nickname });
      } else {

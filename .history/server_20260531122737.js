...
router.post('/edu/friends/request', authenticate, async (req, res) => {
  ...
  db.friendRequests.push({ id: frId, from_id: req.user.id, to_id: target.id, status: 'pending', timestamp: now() });
  // include sender info in notification metadata so clients can show profile actions
  createNotification(target.id, `${req.user.nickname} さんからフレンド申請が届きました`, 'friend_request', { request_id: frId, from_id: req.user.id, from_username: req.user.username, from_nickname: req.user.nickname });
  await saveDb();
...
router.post('/edu/friends/accept', authenticate, async (req, res) => {
  ...
  db.notifications.forEach(n => {
    if (n.request_id === request_id && n.user_id === req.user.id) n.is_read = true;
  });
...
router.post('/edu/friends/reject', authenticate, async (req, res) => {
  const request_id = safeString(req.body.request_id);
  const request = db.friendRequests.find(r => r.id === request_id && r.to_id === req.user.id && r.status === 'pending');
  if (!request) return res.status(404).json({ error: '申請が見つかりません' });
  ...
  createNotification(request.from_id, `${req.user.nickname} さんがフレンド申請を辞退しました`, 'system');
  await saveDb();
  io.emit('friend_update');
  return res.json({ ok: true });
});

router.get('/edu/friends/request_info', authenticate, (req, res) => {
  const request_id = safeString(req.query.request_id);
  if (!request_id) return res.status(400).json({ error: 'request_idが必要です' });
  const request = db.friendRequests.find(r => r.id === request_id);
  if (!request) return res.status(404).json({ error: '申請が見つかりません' });
  const fromUser = db.users.find(u => u.id === request.from_id);
  return res.json({ request: { id: request.id, from_id: request.from_id, to_id: request.to_id, status: request.status, timestamp: request.timestamp }, from: sanitizeUser(fromUser) });
});

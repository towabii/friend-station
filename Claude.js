...
// ✅ 修正3: CORS origin を絞る（あなたのドメインに変更）
app.use(cors({ 
...
const io = new Server(server, { 
    // ✅ 修正2適用
    path: SOCKET_PATH,
    cors: { origin: "https://yourdomain.com", methods: ["GET", "POST", "OPTIONS"], allowedHeaders: "*" },
    allowEIO3: true,
...
// --- Admin API ---
app.post(`${API_BASE}${API_PATH}/admin/login`, (req, res) => { if (req.body.password === "towa.soko0406") { res.json({ token: jwt.sign({ id: "admin", nickname: "Admin", admin: true }, JWT_SECRET) }); } else { res.status(401).json({ error: "Invalid password" }); } });
...
app.post(`${API_BASE}${API_PATH}/admin/users/list`, adminAuth, (req, res) => { db.all(`SELECT * FROM users ORDER BY status ASC, last_id_change DESC`, [], (err, rows) => res.json(rows || [])); });
...
app.post(`${API_BASE}${API_PATH}/admin/posts/list`, adminAuth, (req, res) => { db.all(`SELECT p.*, u.username, u.nickname, (SELECT COUNT(*) FROM post_likes WHERE post_id=p.id) as like_count FROM posts p JOIN users u ON p.user_id=u.id ORDER BY p.timestamp DESC LIMIT 100`, [], (err, rows) => res.json(rows || [])); });
...
app.post(`${API_BASE}${API_PATH}/admin/password_requests/list`, adminAuth, (req, res) => { db.all(`SELECT p.*, u.username, u.full_name FROM password_requests p JOIN users u ON p.user_id = u.id WHERE p.status='pending'`, [], (err, rows) => res.json(rows || [])); });
...
app.post(`${API_BASE}${API_PATH}/admin/messages/list`, adminAuth, (req, res) => { db.all(`SELECT * FROM messages ORDER BY timestamp DESC LIMIT 200`, [], (err, rows) => res.json(rows || [])); });
...
app.post(`${API_BASE}${API_PATH}/admin/backups/list`, adminAuth, (req, res) => { db.all(`SELECT b.id, b.user_id, u.username, u.nickname, b.timestamp FROM backups b JOIN users u ON b.user_id=u.id ORDER BY b.timestamp DESC`, [], (err, rows) => res.json(rows || [])); });
app.post(`${API_BASE}${API_PATH}/admin/backups/view`, adminAuth, (req, res) => { db.get(`SELECT data FROM backups WHERE id=?`, [req.body.id], (err, row) => res.json(row ? JSON.parse(row.data) : [])); });
app.post(`${API_BASE}${API_PATH}/admin/notes/list`, adminAuth, (req, res) => { db.all(`SELECT n.*, u.username, u.nickname, (SELECT COUNT(*) FROM note_likes WHERE note_user_id=n.user_id) as like_count FROM notes n JOIN users u ON n.user_id=u.id ORDER BY n.timestamp DESC LIMIT 100`, [], (err, rows) => res.json(rows || [])); });
app.post(`${API_BASE}${API_PATH}/admin/likes/details`, adminAuth, (req, res) => { 
...
app.post(`${API_BASE}${API_PATH}/user/profile_status`, auth, (req, res) => {
...
app.post(`${API_BASE}${API_PATH}/backup/pull`, (req, res) => {
...
app.post(`${API_BASE}${API_PATH}/notes/list`, auth, (req, res) => { 
...
app.post(`${API_BASE}${API_PATH}/notes/interactions`, auth, (req, res) => {
...
app.post(`${API_BASE}${API_PATH}/groups/list`, auth, (req, res) => {
...
// --- Social & Friends ---
app.post(`${API_BASE}${API_PATH}/sync/messages`, auth, (req, res) => { db.all(`SELECT * FROM messages WHERE room_id LIKE ? AND timestamp > ? ORDER BY timestamp ASC`, [`%${req.user.id}%`, req.body.since], (err, rows) => res.json({ messages: rows || [] })); });
...
app.post(`${API_BASE}${API_PATH}/friends/block_list`, auth, (req, res) => {
...
app.post(`${API_BASE}${API_PATH}/search`, auth, (req, res) => { const q = `%${req.body.query}%`; db.all(`SELECT id,username,nickname,icon_url,status_message FROM users WHERE status='approved' AND (username LIKE ? OR nickname LIKE ?) LIMIT 50`, [q, q], (err, users) => res.json({ users: users || [] })); });
app.post(`${API_BASE}${API_PATH}/search/messages`, auth, (req, res) => { const q = `%${req.body.query}%`; db.all(`SELECT * FROM messages WHERE room_id LIKE ? AND content LIKE ? AND type='text' AND is_deleted=0 ORDER BY timestamp DESC LIMIT 50`, [`%${req.user.id}%`, q], (err, msgs) => res.json({ messages: msgs || [] })); });
app.post(`${API_BASE}${API_PATH}/notifications/list`, auth, (req, res) => { db.all(`SELECT n.*, r.id as request_id, r.status as req_status FROM notifications n LEFT JOIN friend_requests r ON n.from_user=r.from_user AND r.to_user=n.user_id AND r.status='pending' AND n.type='friend_request' WHERE n.user_id=? ORDER BY n.timestamp DESC`, [req.user.id], (err, rows) => res.json(rows || [])); });
...
app.post(`${API_BASE}${API_PATH}/messages/unread_count`, auth, (req, res) => { 
...
app.post(`${API_BASE}${API_PATH}/posts/list`, auth, (req, res) => {
    const q = `
...
app.post(`${API_BASE}${API_PATH}/posts/comments/list`, auth, (req, res) => { db.all(`SELECT c.*, u.nickname, u.icon_url FROM post_comments c JOIN users u ON c.user_id = u.id WHERE c.post_id=? ORDER BY c.timestamp ASC`, [req.body.post_id], (err, rows) => res.json(rows || [])); });
...
app.post(`${API_BASE}${API_PATH}/posts/interactions`, auth, (req, res) => {
...
// ✅ 修正1適用: PORT=443で起動
server.listen(process.env.PORT || 3000, "0.0.0.0", () => console.log("Server Active"));